...
def Pareto(pop_Raw,parameter, QoSAttribute):
    pop=copy.deepcopy(pop_Raw)
...
def Mutation(populationRaw, parameter,tplAll, QoSAttribute):
    ...
    for i in population:
        ...
        if r <= mp:
            ...
            qos = []
            for j in range(len(QoSAttribute)):
                qos.append(tplAll[mutation_position][random.randint(0,len(tplAll[mutation_position])-1)][j])
...
def GA( tplAllRaw, QoSAttribute):
    ...
    for i in range(100):
        ...
        # 突变
        population_Mutation = Mutation(population_Crossover, parameter,tplAllRaw,QoSAttribute)
        ...
        if Fitness(population_Real, tplAll, QoSAttribute) > Fitness(population, tplAll, QoSAttribute):
